#include "uart.h"
#include "string.h"
#include "wdog.h"
#include "timer.h"
#include "gic.h"
#include "mm.h"
#include "task.h"
#include "scheduler.h"
#include "syscall.h"
#include "fs.h"
#include "ramfs.h"

void irq_handler(void)
{
    uint32_t irq_id = gic_ack_irq();
    if (irq_id == TIMER_IRQ_ID) {
        timer_irq_handler();
        scheduler_tick();
    }
    gic_end_irq(irq_id);
}

/* ============================================================
 * Demo User Task (Uses Syscalls)
 * ============================================================ */
void user_task(void)
{
    const char *msg = "Hello from User Task!\r\n";
    long pid = sys_getpid();

    while (1) {
        sys_write(1, "[PID ", 5);
        char pid_str[2];
        pid_str[0] = '0' + pid;
        pid_str[1] = '\0';
        sys_write(1, pid_str, 1);
        sys_write(1, "] ", 2);
        sys_write(1, msg, strlen(msg));
        sys_yield();
        sleep_ms(1000);
    }
}

/* ============================================================
 * Shell Task with VFS commands
 * ============================================================ */
static void exec_cmd(char *cmd);

void shell_task(void)
{
    sys_write(1, "ZKOS> ", 6);
    char buf[128];
    int i = 0;

    while (1) {
        char c = uart_getc();
        if (c == '\r') {
            buf[i] = '\0';
            sys_write(1, "\r\n", 2);
            exec_cmd(buf);
            sys_write(1, "ZKOS> ", 6);
            i = 0;
        } else if (c == '\b' || c == 0x7F) {
            if (i > 0) { i--; sys_write(1, "\b \b", 3); }
        } else {
            if (i < 127) { uart_putc(c); buf[i++] = c; }
        }
    }
}

static void cmd_ls(void)
{
    file_info_t entries[10];
    int count = vfs_listdir("/", entries, 10);
    if (count < 0) {
        uart_puts("Error listing directory.\r\n");
        return;
    }
    uart_puts("NAME                            SIZE\r\n");
    uart_puts("----------------------------------------\r\n");
    for (int i = 0; i < count; i++) {
        uart_puts(entries[i].name);
        for(int s = strlen(entries[i].name); s < 32; s++) uart_putc(' ');
        uart_putint(entries[i].size);
        uart_puts(" bytes\r\n");
    }
}

static void cmd_cat(const char *filename)
{
    int fd = vfs_open(filename, O_RDONLY);
    if (fd < 0) {
        uart_puts("Cannot open file: "); uart_puts(filename); uart_puts("\r\n");
        return;
    }
    char buf[64];
    int bytes;
    uart_puts("--- "); uart_puts(filename); uart_puts(" ---\r\n");
    while ((bytes = vfs_read(fd, buf, sizeof(buf)-1)) > 0) {
        buf[bytes] = '\0';
        uart_puts(buf);
    }
    vfs_close(fd);
    uart_puts("\r\n--- EOF ---\r\n");
}

static void cmd_write(const char *filename, const char *text)
{
    int fd = vfs_open(filename, O_WRONLY | O_CREAT | O_TRUNC);
    if (fd < 0) {
        uart_puts("Cannot open/create file for writing.\r\n");
        return;
    }
    int wrote = vfs_write(fd, text, strlen(text));
    vfs_close(fd);
    if (wrote >= 0) {
        uart_puts("Wrote "); uart_putint(wrote); uart_puts(" bytes.\r\n");
    } else {
        uart_puts("Write failed.\r\n");
    }
}

static void exec_cmd(char *cmd)
{
    char cmd_copy[128];
    strcpy(cmd_copy, cmd);
    
    char *argv[5];
    int argc = 0;
    char *p = cmd_copy;
    
    // Simple basic tokenizer
    while (*p && argc < 5) {
        while (*p == ' ') p++;
        if (!*p) break;
        argv[argc++] = p;
        while (*p && *p != ' ') p++;
        if (*p) { *p = '\0'; p++; }
    }

    if (argc == 0) return;

    if (strcmp(argv[0], "help") == 0) {
        uart_puts("Available commands:\r\n");
        uart_puts("  help            - Show this help\r\n");
        uart_puts("  info            - System info\r\n");
        uart_puts("  meminfo         - Memory stats\r\n");
        uart_puts("  ps              - List tasks\r\n");
        uart_puts("  spawn           - Create task\r\n");
        uart_puts("  ls              - List files\r\n");
        uart_puts("  cat FILE        - Read file\r\n");
        uart_puts("  write FILE TEXT - Write to file (spaces in text not supported in simplistic parser)\r\n");
    }
    else if (strcmp(argv[0], "info") == 0) {
        uart_puts("ZKOS v0.12 - Full Kernel (VFS + RAMFS)\r\n");
    }
    else if (strcmp(argv[0], "meminfo") == 0) {
        uart_puts("Pages: ");
        uart_putint(page_get_used_count()); uart_puts("/");
        uart_putint(page_get_used_count() + page_get_free_count());
        uart_puts("  Heap: "); uart_putint(heap_get_used()); uart_puts("\r\n");
    }
    else if (strcmp(argv[0], "ps") == 0) {
        uart_puts("PID  STATE    NAME\r\n");
        for (int i = 0; i < task_get_count(); i++) {
            struct task_struct *t = task_get(i);
            if (!t) continue;
            uart_puts("  "); uart_putint(t->pid); uart_puts("  ");
            if (t->state == TASK_RUNNING) uart_puts("RUNNING  ");
            else if (t->state == TASK_READY) uart_puts("READY    ");
            else uart_puts("DEAD     ");
            uart_puts(t->name); uart_puts("\r\n");
        }
    }
    else if (strcmp(argv[0], "spawn") == 0) {
        int pid = task_create("usertask", user_task);
        scheduler_add(task_get(pid));
        uart_puts("Spawned usertask PID "); uart_putint(pid); uart_puts("\r\n");
    }
    else if (strcmp(argv[0], "ls") == 0) {
        cmd_ls();
    }
    else if (strcmp(argv[0], "cat") == 0) {
        if (argc < 2) uart_puts("Usage: cat <filename>\r\n");
        else cmd_cat(argv[1]);
    }
    else if (strcmp(argv[0], "write") == 0) {
        if (argc < 3) uart_puts("Usage: write <filename> <text>\r\n");
        else cmd_write(argv[1], argv[2]);
    }
    else {
        uart_puts("Unknown cmd. Type 'help'.\r\n");
    }
}

/* ============================================================
 * Main
 * ============================================================ */
void main(void)
{
    wdog3_disable();

    uart_puts("\r\n=============================\r\n");
    uart_puts("  ZKOS v0.12 - Filesystem\r\n");
    uart_puts("=============================\r\n");

    mmu_init();
    page_alloc_init();
    heap_init();

    gic_init();
    gic_enable_irq(TIMER_IRQ_ID);
    timer_init_irq(10);
    
    vfs_init();
    ramfs_init();

    task_init();
    scheduler_init();

    int shell_pid = task_create("shell", shell_task);
    scheduler_add(task_get(shell_pid));

    asm volatile("msr daifclr, #2");
    scheduler_start(); 
}
